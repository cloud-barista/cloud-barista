...
if [ -f "/etc/kubernetes/kubelet.conf" ]; then
  ...
  R="$(kubectl --kubeconfig=/etc/kubernetes/kubelet.conf annotate node $(hostname | awk \047{print tolower($0)}\047) flannel.alpha.coreos.com/public-ip-overwrite={{PUBLICIP}} --overwrite)"
