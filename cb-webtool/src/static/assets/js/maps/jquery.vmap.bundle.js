...
var JQVMap = function(n) {
    n = n || {};
    var a, o = this, s = JQVMap.maps[n.map];
    if (!s)
        throw new Error('Invalid "' + n.map + '" map parameter. Please make sure you have loaded this map file in your HTML.');
    for (var l in this.selectedRegions = [],
    this.multiSelectRegion = n.multiSelectRegion,
    this.container = n.container,
    this.defaultWidth = s.width,
    this.defaultHeight = s.height,
    this.color = n.color,
    this.selectedColor = n.selectedColor,
    this.hoverColor = n.hoverColor,
    this.hoverColors = n.hoverColors,
    this.hoverOpacity = n.hoverOpacity,
    this.setBackgroundColor(n.backgroundColor),
    this.width = n.container.width(),
    this.height = n.container.height(),
    this.resize(),
    jQuery(window).resize(function() {
        var l = n.container.width()
          , t = n.container.height();
        if (l && t) {
            o.width = l,
            o.height = t,
            o.resize(),
            o.canvas.setSize(o.width, o.height),
            o.applyTransform();
            var e = jQuery.Event("resize.jqvmap");
            jQuery(n.container).trigger(e, [l, t]),
            a && (jQuery(".jqvmap-pin").remove(),
            o.pinHandlers = !1,
            o.placePins(a.pins, a.mode))
        }
    }),
    this.canvas = new VectorCanvas(this.width,this.height,n),
    n.container.append(this.canvas.canvas),
    this.makeDraggable(),
    this.rootGroup = this.canvas.createGroup(!0),
    this.index = JQVMap.mapIndex,
    this.label = jQuery("<div/>").addClass("jqvmap-label").appendTo(jQuery("body")).hide(),
    n.enableZoom && (jQuery("<div/>").addClass("jqvmap-zoomin").text("+").appendTo(n.container),
    jQuery("<div/>").addClass("jqvmap-zoomout").html("&#x2212;").appendTo(n.container)),
    o.countries = [],
    s.paths) {
        var t = this.canvas.createPath({
            path: s.paths[l].path
        });
        t.setFill(this.color),
        t.id = o.getCountryId(l),
        o.countries[l] = t,
        "svg" === this.canvas.mode ? t.setAttribute("class", "jqvmap-region") : jQuery(t).addClass("jqvmap-region"),
        jQuery(this.rootGroup).append(t)
    }
    if (jQuery(n.container).delegate("svg" === this.canvas.mode ? "path" : "shape", "mouseover mouseout", function(l) {
        var t = l.target
          , e = l.target.id.split("_").pop()
          , a = jQuery.Event("labelShow.jqvmap")
          , i = jQuery.Event("regionMouseOver.jqvmap");
        e = e.toLowerCase(),
        "mouseover" === l.type ? (jQuery(n.container).trigger(i, [e, s.paths[e].name]),
        i.isDefaultPrevented() || o.highlight(e, t),
        n.showTooltip && (o.label.text(s.paths[e].name),
        jQuery(n.container).trigger(a, [o.label, e]),
        a.isDefaultPrevented() || (o.label.show(),
        o.labelWidth = o.label.width(),
        o.labelHeight = o.label.height()))) : (o.unhighlight(e, t),
        o.label.hide(),
        jQuery(n.container).trigger("regionMouseOut.jqvmap", [e, s.paths[e].name]))
    }),
    jQuery(n.container).delegate("svg" === this.canvas.mode ? "path" : "shape", "click", function(l) {
        var t = l.target
          , e = l.target.id.split("_").pop()
          , a = jQuery.Event("regionClick.jqvmap");
        if (e = e.toLowerCase(),
        jQuery(n.container).trigger(a, [e, s.paths[e].name]),
        !n.multiSelectRegion && !a.isDefaultPrevented())
            for (var i in s.paths)
                o.countries[i].currentFillColor = o.countries[i].getOriginalFill(),
                o.countries[i].setFill(o.countries[i].getOriginalFill());
        a.isDefaultPrevented() || (o.isSelected(e) ? o.deselect(e, t) : o.select(e, t))
    }),
    n.showTooltip && n.container.mousemove(function(l) {
        if (o.label.is(":visible")) {
            var t = l.pageX - 15 - o.labelWidth
              , e = l.pageY - 15 - o.labelHeight;
            t < 0 && (t = l.pageX + 15),
            e < 0 && (e = l.pageY + 15),
            o.label.css({
                left: t,
    ...
    this.setColors(n.colors),
    this.canvas.canvas.appendChild(this.rootGroup),
    this.applyTransform(),
    this.colorScale = new ColorScale(n.scaleColors,n.normalizeFunction,n.valueMin,n.valueMax),
    n.values && (this.values = n.values,
    this.setValues(n.values)),
    n.selectedRegions)
        if (n.selectedRegions instanceof Array)
            for (var e in n.selectedRegions)
                this.select(n.selectedRegions[e].toLowerCase());
        else
            this.select(n.selectedRegions.toLowerCase());
    if (this.bindZoomButtons(),
    n.pins && (a = {
        pins: n.pins,
        mode: n.pinMode
    },
    this.pinHandlers = !1,
    this.placePins(n.pins, n.pinMode)),
    n.showLabels) {
        this.pinHandlers = !1;
        var i = {};
        for (l in o.countries)
            "function" != typeof o.countries[l] && (n.pins && n.pins[l] || (i[l] = l.toUpperCase()));
        a = {
            pins: i,
            mode: "content"
        },
        this.placePins(i, "content")
    }
    JQVMap.mapIndex++
};
JQVMap.prototype = {
...
function() {
    ...
    jQuery.fn.vectorMap = function(l) {
        ...
        if ("addMap" === l)
            ...
            if ("set" !== l || !n[arguments[1]]) {
                if ("string" == typeof l && "function" == typeof e[l])
                    return e[l].apply(e, Array.prototype.slice.call(arguments, 1));
                for (var a in jQuery.extend(t, l),
                (t.container = this).css({
                    position: "relative",
                    overflow: "hidden"
                }),
                e = new JQVMap(t),
                ...
                this.unbind(".jqvmap"),
                o)
                    t[a] && this.bind(o[a] + ".jqvmap", t[a]);
                var i = jQuery.Event("load.jqvmap");
                return jQuery(t.container).trigger(i, e),
                e
            }
...
ColorScale.arrayToRgb = function(l) {
    for (var t, e = "#", a = 0; a < l.length; a++)
        e += 1 === (t = l[a].toString(16)).length ? "0" + t : t;
    return e
}
,
ColorScale.prototype.getColor = function(l) {
    "function" == typeof this.normalize && (l = this.normalize(l));
    for (var t, e = [], a = 0, i = 0; i < this.colors.length - 1; i++)
        t = this.vectorLength(this.vectorSubtract(this.colors[i + 1], this.colors[i])),
        e.push(t),
        a += t;
    ...
    for (i = 0; i < e.length; i++)
        e[i] *= o;
    for (i = 0,
    l -= this.minValue; 0 <= l - e[i]; )
        l -= e[i],
        i++;
    for (n = i === this.colors.length - 1 ? this.vectorToNum(this.colors[i]).toString(16) : this.vectorToNum(this.vectorAdd(this.colors[i], this.vectorMult(this.vectorSubtract(this.colors[i + 1], this.colors[i]), l / e[i]))).toString(16); n.length < 6; )
        n = "0" + n;
    return "#" + n
}
,
...
JQVMap.prototype.applyTransform = function() {
    var l, t, e, a;
    e = this.defaultWidth * this.scale <= this.width ? (l = (this.width - this.defaultWidth * this.scale) / (2 * this.scale),
    (this.width - this.defaultWidth * this.scale) / (2 * this.scale)) : (l = 0,
    (this.width - this.defaultWidth * this.scale) / this.scale),
    a = this.defaultHeight * this.scale <= this.height ? (t = (this.height - this.defaultHeight * this.scale) / (2 * this.scale),
    (this.height - this.defaultHeight * this.scale) / (2 * this.scale)) : (t = 0,
    (this.height - this.defaultHeight * this.scale) / this.scale),
    this.transY > t ? this.transY = t : this.transY < a && (this.transY = a),
    this.transX > l ? this.transX = l : this.transX < e && (this.transX = e),
...
JQVMap.prototype.deselect = function(l, t) {
    if (l = l.toLowerCase(),
    t = t || jQuery("#" + this.getCountryId(l))[0],
    this.isSelected(l))
        this.selectedRegions.splice(this.selectIndex(l), 1),
        jQuery(this.container).trigger("regionDeselect.jqvmap", [l]),
        t.currentFillColor = t.getOriginalFill(),
        t.setFill(t.getOriginalFill());
    else
        for (var e in this.countries)
            this.selectedRegions.splice(this.selectedRegions.indexOf(e), 1),
            this.countries[e].currentFillColor = this.color,
            this.countries[e].setFill(this.color)
}
,
...
JQVMap.prototype.getPins = function() {
    var l = this.container.find(".jqvmap-pin")
      , i = {};
    return jQuery.each(l, function(l, t) {
        var e = (t = jQuery(t)).attr("for").toLowerCase()
          , a = t.html();
        i[e] = a
    }),
    JSON.stringify(i)
...
JQVMap.prototype.highlight = function(l, t) {
    t = t || jQuery("#" + this.getCountryId(l))[0],
    this.hoverOpacity ? t.setOpacity(this.hoverOpacity) : this.hoverColors && l in this.hoverColors ? (t.currentFillColor = t.getFill() + "",
    t.setFill(this.hoverColors[l])) : this.hoverColor && (t.currentFillColor = t.getFill() + "",
    t.setFill(this.hoverColor))
}
,
JQVMap.prototype.isSelected = function(l) {
    return 0 <= this.selectIndex(l)
}
,
JQVMap.prototype.makeDraggable = function() {
    var t, e, o, s, r, h, L, p, c, a = !1, u = this;
    u.isMoving = !1,
    u.isMovingTimeout = !1,
    this.container.mousemove(function(l) {
        return a && (u.transX -= (t - l.pageX) / u.scale,
        u.transY -= (e - l.pageY) / u.scale,
        u.applyTransform(),
        t = l.pageX,
        e = l.pageY,
        u.isMoving = !0,
        u.isMovingTimeout && clearTimeout(u.isMovingTimeout),
        u.container.trigger("drag")),
        !1
        ...
        return a = !1,
        clearTimeout(u.isMovingTimeout),
        u.isMovingTimeout = setTimeout(function() {
            u.isMoving = !1
        }, 100),
        !1
    }).mouseout(function() {
        if (a && u.isMoving)
            return clearTimeout(u.isMovingTimeout),
            u.isMovingTimeout = setTimeout(function() {
                a = !1,
                u.isMoving = !1
            }, 100),
    ...
    jQuery(this.container).bind("touchmove", function(l) {
        var t, e, a, i, n = l.originalEvent.touches;
        if (1 === n.length) {
            if (1 === o) {
                if (p === n[0].pageX && c === n[0].pageY)
                    return;
                a = u.transX,
                i = u.transY,
                u.transX -= (p - n[0].pageX) / u.scale,
                u.transY -= (c - n[0].pageY) / u.scale,
                u.applyTransform(),
                a === u.transX && i === u.transY || l.preventDefault(),
                u.isMoving = !0,
                u.isMovingTimeout && clearTimeout(u.isMovingTimeout)
            }
            p = n[0].pageX,
            ...
            2 === n.length && (2 === o ? (e = Math.sqrt(Math.pow(n[0].pageX - n[1].pageX, 2) + Math.pow(n[0].pageY - n[1].pageY, 2)) / h,
            u.setScale(L * e, s, r),
            l.preventDefault()) : (t = jQuery(u.container).offset(),
            s = n[0].pageX > n[1].pageX ? n[1].pageX + (n[0].pageX - n[1].pageX) / 2 : n[0].pageX + (n[1].pageX - n[0].pageX) / 2,
            r = n[0].pageY > n[1].pageY ? n[1].pageY + (n[0].pageY - n[1].pageY) / 2 : n[0].pageY + (n[1].pageY - n[0].pageY) / 2,
            s -= t.left,
            r -= t.top,
            L = u.scale,
            h = Math.sqrt(Math.pow(n[0].pageX - n[1].pageX, 2) + Math.pow(n[0].pageY - n[1].pageY, 2))));
        o = n.length
    }),
...
JQVMap.prototype.placePins = function(l, t) {
    var i = this;
    if (t && ("content" === t || "id" === t) || (t = "content"),
    "content" === t ? jQuery.each(l, function(l, t) {
        if (0 !== jQuery("#" + i.getCountryId(l)).length) {
            var e = i.getPinId(l)
              , a = jQuery("#" + e);
            0 < a.length && a.remove(),
            i.container.append('<div id="' + e + '" for="' + l + '" class="jqvmap-pin" style="position:absolute">' + t + "</div>")
        }
        ...
        if (0 !== jQuery("#" + i.getCountryId(l)).length) {
            var e = i.getPinId(l)
              , a = jQuery("#" + e);
            0 < a.length && a.remove(),
            i.container.append('<div id="' + e + '" for="' + l + '" class="jqvmap-pin" style="position:absolute"></div>'),
            a.append(jQuery("#" + t))
        }
    }),
    this.positionPins(),
    !this.pinHandlers) {
        ...
        this.container.bind("zoomIn", e).bind("zoomOut", e).bind("drag", e)
    }
}
,
JQVMap.prototype.positionPins = function() {
    var h = this
      , l = this.container.find(".jqvmap-pin");
    jQuery.each(l, function(l, t) {
        t = jQuery(t);
        var e = h.getCountryId(t.attr("for").toLowerCase())
          , a = jQuery("#" + e)
          , i = document.getElementById(e).getBBox()
          , n = a.position()
          , o = h.scale
          , s = n.left + i.width / 2 * o - t.width() / 2
          , r = n.top + i.height / 2 * o - t.height() / 2;
        t.css("left", s).css("top", r)
    })
}
,
JQVMap.prototype.removePin = function(l) {
    l = l.toLowerCase(),
    jQuery("#" + this.getPinId(l)).remove()
}
,
...
JQVMap.prototype.reset = function() {
    for (var l in this.countries)
        this.countries[l].setFill(this.color);
...
JQVMap.prototype.select = function(l, t) {
    l = l.toLowerCase(),
    t = t || jQuery("#" + this.getCountryId(l))[0],
    this.isSelected(l) || (this.multiSelectRegion ? this.selectedRegions.push(l) : this.selectedRegions = [l],
    jQuery(this.container).trigger("regionSelect.jqvmap", [l]),
    this.selectedColor && t && (t.currentFillColor = this.selectedColor,
    t.setFill(this.selectedColor)))
}
,
JQVMap.prototype.selectIndex = function(l) {
    l = l.toLowerCase();
    for (var t = 0; t < this.selectedRegions.length; t++)
        if (l === this.selectedRegions[t])
            return t;
    return -1
}
,
...
JQVMap.prototype.setColors = function(l, t) {
    if ("string" == typeof l)
        this.countries[l].setFill(t),
        this.countries[l].setAttribute("original", t);
    else {
        var e = l;
        for (var a in e)
            this.countries[a] && (this.countries[a].setFill(e[a]),
            this.countries[a].setAttribute("original", e[a]))
    }
}
,
JQVMap.prototype.setNormalizeFunction = function(l) {
    this.colorScale.setNormalizeFunction(l),
...
JQVMap.prototype.setScaleColors = function(l) {
    this.colorScale.setColors(l),
    this.values && this.setValues(this.values)
}
,
JQVMap.prototype.setValues = function(l) {
    var t, e = 0, a = Number.MAX_VALUE;
    for (var i in l)
        i = i.toLowerCase(),
        t = parseFloat(l[i]),
        isNaN(t) || (e < t && (e = l[i]),
        t < a && (a = t));
    a === e && e++,
    this.colorScale.setMin(a),
    this.colorScale.setMax(e);
    var n = {};
    for (i in l)
        i = i.toLowerCase(),
        t = parseFloat(l[i]),
        n[i] = isNaN(t) ? this.color : this.colorScale.getColor(t);
    this.setColors(n),
    this.values = l
}
,
JQVMap.prototype.unhighlight = function(l, t) {
    l = l.toLowerCase(),
    (t = t || jQuery("#" + this.getCountryId(l))[0]).setOpacity(1),
    t.currentFillColor && t.setFill(t.currentFillColor)
}
,
JQVMap.prototype.zoomIn = function() {
    ...
    if (l.zoomCurStep < l.zoomMaxStep) {
        l.transX -= (l.width / l.scale - l.width / (l.scale * l.zoomStep)) / 2,
        l.transY -= (l.height / l.scale - l.height / (l.scale * l.zoomStep)) / 2,
        l.setScale(l.scale * l.zoomStep),
        l.zoomCurStep++;
        var e = jQuery("#zoomSlider");
        e.css("top", parseInt(e.css("top"), 10) - t),
        l.container.trigger("zoomIn")
    }
}
,
JQVMap.prototype.zoomOut = function() {
    var l = this
      , t = (jQuery("#zoom").innerHeight() - 12 - 30 - 6 - 7 - 6) / (this.zoomMaxStep - this.zoomCurStep);
    if (1 < l.zoomCurStep) {
        l.transX += (l.width / (l.scale / l.zoomStep) - l.width / l.scale) / 2,
        l.transY += (l.height / (l.scale / l.zoomStep) - l.height / l.scale) / 2,
        l.setScale(l.scale / l.zoomStep),
        l.zoomCurStep--;
        var e = jQuery("#zoomSlider");
        e.css("top", parseInt(e.css("top"), 10) + t),
        l.container.trigger("zoomOut")
    }
}
,
VectorCanvas.prototype.applyTransformParams = function(l, t, e) {
    "svg" === this.mode ? this.rootGroup.setAttribute("transform", "scale(" + l + ") translate(" + t + ", " + e + ")") : (this.rootGroup.coordorigin = this.width - t + "," + (this.height - e),
    this.rootGroup.coordsize = this.width / l + "," + this.height / l)
}
,
VectorCanvas.prototype.createGroup = function(l) {
    var t;
    return "svg" === this.mode ? t = this.createSvgNode("g") : ((t = this.createVmlNode("group")).style.width = this.width + "px",
...
VectorCanvas.prototype.createPath = function(l) {
    var t;
    if ("svg" === this.mode)
        (t = this.createSvgNode("path")).setAttribute("d", l.path),
        null !== this.params.borderColor && t.setAttribute("stroke", this.params.borderColor),
        0 < this.params.borderWidth && (t.setAttribute("stroke-width", this.params.borderWidth),
        t.setAttribute("stroke-linecap", "round"),
        t.setAttribute("stroke-linejoin", "round")),
        0 < this.params.borderOpacity && t.setAttribute("stroke-opacity", this.params.borderOpacity),
        t.setFill = function(l) {
            this.setAttribute("fill", l),
            null === this.getAttribute("original") && this.setAttribute("original", l)
        }
        ,
        t.getFill = function() {
        ...
        t.style.height = this.height + "px",
        t.fillcolor = JQVMap.defaultFillColor,
        t.stroked = !1,
        t.path = VectorCanvas.pathSvgToVml(l.path);
        var e = this.createVmlNode("skew");
        e.on = !0,
        e.matrix = "0.01,0,0,0.01,0,0",
        e.offset = "0,0",
        t.appendChild(e);
        var a = this.createVmlNode("fill");
        t.appendChild(a),
        t.setFill = function(l) {
            this.getElementsByTagName("fill")[0].color = l,
            null === this.getAttribute("original") && this.setAttribute("original", l)
        }
        ,
        t.getFill = function() {
...
VectorCanvas.prototype.pathSvgToVml = function(l) {
    var n, o, s = "", r = 0, h = 0;
    return l.replace(/([MmLlHhVvCcSs])((?:-?(?:\d+)?(?:\.\d+)?,?\s?)+)/g, function(l, t, e) {
        (e = e.replace(/(\d)-/g, "$1,-").replace(/\s+/g, ",").split(","))[0] || e.shift();
...
VectorCanvas.prototype.setSize = function(l, t) {
    if ("svg" === this.mode)
        this.canvas.setAttribute("width", l),
        this.canvas.setAttribute("height", t);
    else if (this.canvas.style.width = l + "px",
    this.canvas.style.height = t + "px",
    this.canvas.coordsize = l + " " + t,
    this.canvas.coordorigin = "0 0",
    this.rootGroup) {
        for (var e = this.rootGroup.getElementsByTagName("shape"), a = 0, i = e.length; a < i; a++)
            e[a].coordsize = l + " " + t,
            e[a].style.width = l + "px",
            e[a].style.height = t + "px";
        this.rootGroup.coordsize = l + " " + t,
        this.rootGroup.style.width = l + "px",
